...
1) Buildkit - the `docker buildx build` command would be used in place of `docker build` .  `docker build` is now aliased to use Buildkit but there are some [subtle differences](https://docs.docker.com/build/builders/#difference-between-docker-build-and-docker-buildx-build) to calling Buildkit directly via docker buildx build. 
...
4) CI/CD tools - its quite likely you would start development of a project within github where you would include a docker file to provide a stable and reproducible development environment for yourself and others as part of your development process.  In addition, CI/CD tools like Github actions would be used to automate the process of performing and checking builds.  Alternative CI/CD tools like Buildkite might be used on larger scale projects.
...
(or `code Dockerfile` if you’re using VS Code / Codespaces)
...
1. **`FROM mcr.microsoft.com/dotnet/core/sdk:2.2`**
   Uses Microsoft’s .NET Core SDK 2.2 image as the base (so `dotnet` build tools are available). ([Microsoft Learn][1])
...
6. **`COPY . .`**
   Copies the rest of the source tree from your local `src` directory into the image’s `/src`. ([Microsoft Learn][1])
...
10. **`EXPOSE 80`**
    Documents that the container listens on port 80 (the app’s HTTP port). ([Microsoft Learn][1])
...
### 8) Run the container in the background, mapping host port 8080 → container port 80
...
These instructions are based on this [tutorial](https://learn.microsoft.com/en-us/training/modules/intro-to-containers/7-exercise-deploy-docker-image-to-container-instance).  "Exercise - Deploy a Docker image to an Azure Container Instance - Training | Microsoft Learn".  A more advanced version of this tutorial that uses the Azure Cloud CLI is located [here](https://learn.microsoft.com/en-us/training/modules/build-and-store-container-images/)  
...
**Why:** Azure Container Instances (ACI) needs a registry it can pull your image from; ACR is Azure’s native private registry. ([Microsoft Learn][1])
...
**Why:** Tagging tells Docker “this local image should be pushed to *that* registry/repository name.” ([Microsoft Learn][1])
...
**Why:** This confirms you’ll push the correct image to the correct registry address. ([Microsoft Learn][1])
...
**Why:** This is the “deployment” step—ACI pulls your image and runs it as a managed container with a public endpoint. ([Microsoft Learn][1])

In Azure portal:

* Create resource → **Container Instances**
* Basics tab:
...
* Review + create → Create. ([Microsoft Learn][1])

---

### 8) Find the container’s public FQDN so you can reach it from the internet
...
**Why:** This validates the full pipeline: local image → ACR → ACI → public endpoint. ([Microsoft Learn][1])
...
* If you can’t see an FQDN/Public endpoint, double-check **Networking type = Public** and **Port 80** is open. ([Microsoft Learn][1])
